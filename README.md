...
🥞 **Node.js**工具，用于与**PancakeSwap v2 DEX**集成，提供**滑点估算**、**价格影响计算**和**流动性池监控**功能。

> **📋 任务评估**: 查看[任务评估表](./TASK_EVALUATION.md)，了解针对PancakeSwap集成任务的详细检查清单。

## 思考点
PancakeSwap v2 使用AMM公式计算滑点，需要校对公式计算是否正确。
再通过 PancakeSwap Dapp页面，选择指定v2路由，验证计算的正确性。
可深入展开的工作：扩展匹配不同的pool，支持更多的计算，研究最优路由算法。

## ✨ 功能特性

### 1. 滑点估算 📉
- 通过模拟**PancakeSwap**上的卖出操作来实现精确的滑点计算
- 使用**PancakeSwap的AMM公式**进行计算：*实际输出 = (输入量 × 9975 × 储备输出) / (储备输入 × 10000 + 输入量 × 9975)*
- 考虑**0.25%的交易手续费**
- 支持任意数量的代币卖出模拟
...
### 2. 价格影响计算 💥
- 计算大额卖出对代币价格的具体影响百分比
- **默认计算**：卖出市值的 **0.5%** 和 **5%** 时的价格影响
- 基于真实的**AMM数学模型**，结合流动性深度分析
- **精确手续费**: 所有计算基于PancakeSwap v2正确的0.25%手续费
- 提供详细的分析报告
- 支持自定义市值百分比
